...
__weak void MC_APP_PostMediumFrequencyHook_M1(void)
{
  ...
  // (void)SPDPOT_Run(&SpeedPotentiometer_M1, rawValue); // SPDPOT disabled to prevent conflicts
    ...
    case THR_RUNNING:
      if (throttlePct == 0) {
        ...
        if (state == RUN) {
          ...
          MCI_ExecSpeedRamp(pMCI[M1], targetSpeedUnit, 500); /* 500 ms smooth transition */
